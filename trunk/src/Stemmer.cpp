...
int Stemmer::ends(char * str){
	...
	int length = str[0];
	if (str[length] != b[k]) return false; /* tiny speed-up */
	if (length > k-k0+1) return false;
	if (memcmp(b+k-length+1,str+1,length) != 0) return false;
	j = k-length;
	return true;
}
...
/* applies the stem function over the parameter file.
   Prints on the console the result.
*/
std::string Stemmer::stemword(std::string w){
